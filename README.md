...
This catalog uses a fully local AI search engine (all-MiniLM-L12). It runs entirely in your browser, keeping everything fast and private without relying on any external servers.
...
*   **Search Time:** When you type, your search becomes a fingerprint too. The AI overlays your fingerprint onto the tea database, scales the similarity into a human-friendly percentage, and returns the best matches!

### What it can do:

*   **Vibe & Scenario Matching:** Search for "cozy rainy day," "3pm slump," or "stomach ache."
*   **Hybrid Search:** Blends deep AI understanding with exact keyword boosts to get the most accurate results.
...
*   **AI Badges:** Top results show visual badges (like `stress relief` or `soothing`) explaining exactly why the AI chose them.

### Customizing

Teach the AI new tricks by editing `teas.json` or using the Dev Admin Panel. Just add keywords to a tea's `aiSemanticProfile` field and run `npm run update-ai` to recalculate its fingerprint!
...
*   **[all-MiniLM-L12-v2](https://huggingface.co/Xenova/all-MiniLM-L12-v2)**: The AI model that gives the catalog its semantic understanding.
